...
`jarvis-release.json` pins the exact frontend source in `braydenparker999/jarvis`. The workflow checks out that revision into `.jarvis-source`, runs its tests, and packages `public/`. Keeping that repository retains its Cloudflare deployment relationship and publication issue #2. The original frontend at https://gray-meadow-09216fd10.1.azurestaticapps.net/ remains a backup. Existing MissionaryTube source is retained in this repository for rollback and is **not** included in the Jarvis deployment.
