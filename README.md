...
Quick AI at `/quick-ai/` sends requests to the existing Cloudflare Worker. The Worker stores provider keys as secrets and gates each model on verified free-tier settings. The browser keeps chats and images locally. The Azure workflow publishes a credential-free configuration, deletes the old exposed-key blob before upload, and requires Worker v2 before frontend cutover. The old frontend Groq Actions secret is no longer used; rotate it after confirming no other consumer uses it.
