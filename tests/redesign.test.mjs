...
test("nothing in the UI invents content, availability or telemetry", () => {
  ...
  // "Trending" is allowed in exactly one place, because there it is a real
  // Invidious response rather than a label invented over arbitrary items.
  assert.match(html, /client\.trending\(\{signal:controller\.signal\}\)/);
  assert.equal(/trending/i.test(html.replace(/[Tt]rending/g, (m, i) =>
...
test("a sector is only shown as populated when a provider exposes it", () => {
  // The taxonomy still owns the decision. What it is now told is which sectors
  // the app provides itself, because YouTube has no add-on behind it.
  assert.match(html, /function hubSectors\(\)\{return AstraHub\.buildHub\(manifests\(\),\{builtIn:youtubeEnabled\(\)\?\['youtube'\]:\[\]\}\)\}/);
  assert.match(html, /const sectors=hubSectors\(\)/);
  assert.equal(/AstraHub\.buildHub\(manifests\(\)\)/.test(html), false,
    "every caller goes through the one helper, so built-in coverage cannot drift");
  ...
  assert.match(html, /:`data-nav="addons"`/, "an unavailable add-on sector routes to Add-ons");
  assert.match(html, /if\(!sector\.available\)return sector\.builtIn\?nav\('settings','youtube'\):nav\('addons'\)/,
    "an unavailable built-in sector routes to its own settings instead");
});

test("podcasts and radio are gone from navigation and product scope", () => {
  // The taxonomy owns the decision; the app asks it rather than re-listing it.
  assert.match(hubSource, /var OUT_OF_SCOPE = \[/);
  for (const type of ["podcast", "radio"]) {
    assert.match(hubSource, new RegExp(`"${type}"`), `${type} must be named as out of scope`);
  }
  assert.equal(/id: "(?:podcast|radio)"/.test(hubSource), false, "no sector may remain");
  ...
  // The four destinations are still Home, Search, Library and Settings.
  assert.match(html, /const navs=\[\['home','home','Home'\],\['search','search','Search'\],\['library','library','Library'\],\['settings','settings','Settings'\]\]/);
  for (const gone of ["'Podcasts'", "'Radio'"]) {
    assert.equal(html.includes(gone), false, `${gone} must not be a label in the app`);
  }
});

test("YouTube plays natively, and no iframe or embed survives anywhere", () => {
  // The whole point of the Invidious provider: real stream URLs in the real
  // player, never a YouTube embed. An iframe is not a fallback here, it is the
  // thing that does not work on the target device.
  assert.equal(/<iframe|youtube-nocookie|youtube\.com\/embed|YT\.Player|IFrame ?API/i.test(html), false,
    "no embed of any kind may reappear");
  assert.equal(/<iframe/i.test(css), false);
  // A bare YouTube id from an add-on is resolved through the same provider.
  assert.match(html, /if\(s\.kind==='youtube'\)\{openAddonYouTube\(entry\);return\}/);
  assert.match(html, /async function openAddonYouTube\(entry\)/);
  // No API key, and no Google endpoint called directly by the app.
  assert.equal(/(youtube|yt)[_-]?api[_-]?key/i.test(html), false);
  assert.equal(/googleapis\.com/.test(html), false);
  // Instance URLs live in one configuration module, never in the app shell.
  // The only address index.html may contain is the field's own placeholder.
...
test("modal icon buttons and switches expose names and live state", () => {
  ...
  for (const gone of ["autoplayNext", "preferCached", "hdrPreference", "maxResolution"]) {
    assert.equal(html.includes(gone), false, `${gone} no longer decides anything`);
  }
  // `autoFailover` is an engine option, never a preference. The one place it is
  // switched on is the YouTube ladder, where the alternatives are the same
  // video in different deliveries rather than competing releases.
  assert.equal(/data-setting="autoFailover"|settings\.autoFailover/.test(html), false,
    "autoFailover is not a setting");
  assert.match(html, /autoFailover:!!ladder/);
  assert.match(html, /const ladder=isYouTubeEntry\(entry\)\?player\.sources\.filter\(isYouTubeEntry\):null/);
