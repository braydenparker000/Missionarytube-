 ...
 * A Piped instance.
 *
 * The difference that matters is modelled faithfully: Piped proxies its own
 * stream URLs, so they point back at this server and carry CORS headers. That
 * is why a Piped instance works for a browser-only static site where a raw
 * Google URL would not, and it is the arrangement the app actually ships on.
 */
export async function startPiped({ media, behaviour = { mode: "ok" }, name = "piped" }) {
  const byId = new Map(media.map((video) => [video.id, video]));
  const requests = [];
  const state = behaviour;
  let origin = "";
  ...
  function row(video) {
    return {
      ...
      type: "stream",
      title: video.title,
      thumbnail: `${origin}/vi/${video.id}/hqdefault.jpg`,
      uploaderName: video.author,
      uploaderUrl: "/channel/UCbenchbenchbenchbenchbe",
      uploadedDate: "3 years ago",
      shortDescription: `${video.title}: a generated clip used to prove playback really happens.`,
      duration: video.seconds,
      views: 100000 + video.seconds * 137,
      // Milliseconds, as Piped really reports it.
  ...
  function streams(video) {
    return {
      title: video.title,
      description: `${video.title}<br>Served by the mock Piped instance.<p>Second paragraph.</p>`,
      uploadDate: "2020-09-13",
      uploader: video.author,
      uploaderUrl: "/channel/UCbenchbenchbenchbenchbe",
      thumbnailUrl: `${origin}/vi/${video.id}/hqdefault.jpg`,
      hls: null,
      dash: null,
      duration: video.seconds,
      views: 100000,
      ...
      proxyUrl: origin,
      audioStreams: [],
      // Muxed only, which is the ordinary Piped case and exactly what a plain
      // <video src> needs.
      videoStreams: video.progressive.map((entry) => ({
        url: `${origin}/videoplayback?id=${video.id}&itag=${entry.itag}`,
        format: "WEBM",
        quality: `${entry.height}p`,
        mimeType: "video/webm",
        codec: null,
        videoOnly: false,
        itag: Number(entry.itag),
        bitrate: entry.height * 2000,
        width: entry.height === 360 ? 640 : 1280,
        height: entry.height,
        fps: 25
      })),
      relatedStreams: [],
      subtitles: [
        { url: `${origin}/subtitles/${video.id}`, mimeType: "text/vtt", name: "English", code: "en", autoGenerated: false }
      ],
      chapters: [],
      previewFrames: []
    };
  ...
  const started = await listen((request, response) => {
    ...
    if (url.pathname === "/trending") {
      cors(response);
      return json(response, media.map(row));
    }
    if (url.pathname === "/search") {
      const wanted = (url.searchParams.get("q") || "").toLowerCase();
      cors(response);
      return json(response, {
        items: media.filter((video) => video.title.toLowerCase().includes(wanted)).map(row),
        nextpage: null
      });
    }
    const streamsMatch = /^\/streams\/([A-Za-z0-9_-]{11})$/.exec(url.pathname);
    if (streamsMatch) {
      const video = byId.get(streamsMatch[1]);
      cors(response);
      if (!video) return json(response, { error: "This video is unavailable" }, 500);
      return json(response, streams(video));
    }
    const subsMatch = /^\/subtitles\/([A-Za-z0-9_-]{11})$/.exec(url.pathname);
    if (subsMatch) {
      const video = byId.get(subsMatch[1]);
      if (!video) return void response.writeHead(404).end();
      return sendFile(request, response, video.captions);
    }
    if (url.pathname === "/videoplayback") {
      ...
      const entry = video && video.progressive.find((item) => item.itag === itag);
      if (!entry) return void response.writeHead(404).end();
      // Proxied by the instance, so unlike Google's host this one does send CORS.
      return sendFile(request, response, entry.path);
    }
    const thumbMatch = /^\/vi\/([A-Za-z0-9_-]{11})\//.exec(url.pathname);
    if (thumbMatch) {
