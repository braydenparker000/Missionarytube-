...
Artwork is real in these captures. `tests/fixtures/artwork/` holds nine committed
synthetic images — geometric compositions generated by a canvas script, not photographs and not
anyone's key art, each stamped "SYNTHETIC FIXTURE". One backdrop is deliberately high-key so the
scrim's contrast is stressed rather than flattered by a dark image, and two titles keep a
deliberately unresolvable poster URL so the failure path stays in evidence too.
